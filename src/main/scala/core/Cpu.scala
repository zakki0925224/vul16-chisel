package core

import chisel3._
import chisel3.util._
import core.Consts._

class Cpu extends Module {
    def signExtend(x: UInt, fromWidth: Int, toWidth: Int): UInt = {
        val sign = x(fromWidth - 1)
        val ext  = Fill(toWidth - fromWidth, sign)
        Cat(ext, x)
    }

    // (op, rd, rs1, rs2, imm/offset)
    def decode(inst: UInt): (Opcode.Type, UInt, UInt, UInt, UInt) = {
        val op  = Opcode.fromInst(inst)
        val fmt = Opcode.toFormatType(op)

        val rd  = WireDefault(0.U(3.W))
        val rs1 = WireDefault(0.U(3.W))
        val rs2 = WireDefault(0.U(3.W))
        val imm = WireDefault(0.U(WORD_LEN.W))

        switch(fmt) {
            is(FormatType.R) {
                rd  := inst(10, 8)
                rs1 := inst(7, 5)
                rs2 := inst(4, 2)
                imm := 0.U(WORD_LEN.W)
            }
            is(FormatType.I) {
                rd  := inst(10, 8)
                rs1 := inst(7, 5)
                imm := signExtend(inst(4, 0), 5, WORD_LEN)
            }
            is(FormatType.J) {
                rd  := inst(10, 8)
                imm := signExtend(inst(7, 0), 8, WORD_LEN)
            }
            is(FormatType.B) {
                rs1 := inst(10, 8)
                rs2 := inst(7, 5)
                imm := signExtend(inst(4, 0), 5, WORD_LEN)
            }
        }

        val out_rd  = WireDefault(0.U(3.W))
        val out_rs1 = WireDefault(0.U(3.W))
        val out_rs2 = WireDefault(0.U(3.W))
        val out_imm = WireDefault(0.U(WORD_LEN.W))

        switch(fmt) {
            is(FormatType.R) {
                out_rd  := rd
                out_rs1 := rs1
                out_rs2 := rs2
                out_imm := 0.U(WORD_LEN.W)
            }
            is(FormatType.I) {
                out_rd  := rd
                out_rs1 := rs1
                out_rs2 := 0.U
                out_imm := imm
            }
            is(FormatType.J) {
                out_rd  := rd
                out_rs1 := 0.U
                out_rs2 := 0.U
                out_imm := imm
            }
            is(FormatType.B) {
                out_rd  := 0.U
                out_rs1 := rs1
                out_rs2 := rs2
                out_imm := imm
            }
        }

        (op, out_rd, out_rs1, out_rs2, out_imm)
    }

    val io = IO(new Bundle {
        val memDataAddr  = Output(UInt(WORD_LEN.W))
        val memDataIn    = Output(UInt(BYTE_LEN.W))
        val memDataOut   = Input(UInt(BYTE_LEN.W))
        val memDataWrite = Output(Bool())
        val memDataReq   = Output(Bool())
        val memDataDone  = Input(Bool())
        val memInstReq   = Output(Bool())
        val memInstDone  = Input(Bool())

        val inst   = Input(UInt(WORD_LEN.W))
        val pc     = Output(UInt(WORD_LEN.W))
        val gpRegs = Output(Vec(NUM_GP_REGS, UInt(WORD_LEN.W)))

        val debugHalt = Input(Bool())
        val debugStep = Input(Bool())
    })

    val memDataAddrReg  = RegInit(0.U(WORD_LEN.W))
    val memDataInReg    = RegInit(0.U(BYTE_LEN.W))
    val memDataWriteReg = RegInit(false.B)
    io.memDataAddr  := memDataAddrReg
    io.memDataIn    := memDataInReg
    io.memDataWrite := memDataWriteReg

    // general purpose registers
    val gpRegs = VecInit(Seq.fill(NUM_GP_REGS - 1)(Module(new Register()).io))
    for (i <- 0 until (NUM_GP_REGS - 1)) {
        gpRegs(i).in    := gpRegs(i).out
        gpRegs(i).write := false.B
    }

    def readGpReg(reg: UInt): UInt = {
        val dataVec = Wire(Vec(NUM_GP_REGS, UInt(WORD_LEN.W)))
        dataVec(0) := 0.U
        for (i <- 1 until NUM_GP_REGS) {
            dataVec(i) := gpRegs(i - 1).out
        }
        dataVec(reg)
    }

    def writeGpReg(reg: UInt, value: UInt): Unit = {
        when(reg =/= 0.U) {
            for (i <- 1 until NUM_GP_REGS) {
                when(reg === i.U) {
                    gpRegs(i - 1).in    := value
                    gpRegs(i - 1).write := true.B
                }
            }
        }
    }

    io.gpRegs(0) := 0.U
    for (i <- 1 until NUM_GP_REGS) {
        io.gpRegs(i) := gpRegs(i - 1).out
    }

    val pc = Module(new Register(START_ADDR.U(WORD_LEN.W)))
    io.pc       := pc.io.out
    pc.io.in    := pc.io.out
    pc.io.write := false.B

    val memDataReq = RegInit(false.B)
    val memInstReq = RegInit(false.B)
    io.memDataReq := memDataReq
    io.memInstReq := memInstReq

    val alu = Module(new Alu())
    alu.io.a  := 0.U(WORD_LEN.W)
    alu.io.b  := 0.U(WORD_LEN.W)
    alu.io.op := AluOpcode.Add

    // state machine
    val sFetch :: sDecode :: sExec :: sExec2 :: Nil = Enum(4)
    val state                                       = RegInit(sFetch)

    val lswSFetch :: lswSFirstByte :: lswSSecondByte :: Nil = Enum(3)
    val lswState                                            = RegInit(lswSFetch)

    val op  = RegInit(Opcode.Add)
    val rd  = RegInit(0.U(3.W))
    val rs1 = RegInit(0.U(3.W))
    val rs2 = RegInit(0.U(3.W))
    val imm = RegInit(0.U(WORD_LEN.W))

    val dataBuf = RegInit(0.U(BYTE_LEN.W))

    val allowStep = !io.debugHalt || (io.debugHalt && io.debugStep)

    when(allowStep) {
        switch(state) {
            is(sFetch) {
                // request instruction
                memInstReq := true.B
                state      := sDecode
            }
            is(sDecode) {
                when(io.memInstDone) {
                    memInstReq := false.B
                    val (dOp, dRd, dRs1, dRs2, dImm) = decode(io.inst)
                    op    := dOp
                    rd    := dRd
                    rs1   := dRs1
                    rs2   := dRs2
                    imm   := dImm
                    state := sExec
                    // printf(p"pc=${pc.io.out}, op=$op, rd=$rd, rs1=$rs1, rs2=$rs2, imm=$imm\n")
                }
            }
            is(sExec) {
                when(op === Opcode.Add) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Add
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Addi) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Add
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Sub) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Sub
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.And) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.And
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Andi) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.And
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Or) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Or
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Ori) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Or
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Xor) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Xor
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Xori) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Xor
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Sll) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Sll
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Slli) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Sll
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Srl) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Srl
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Srli) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Srl
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Sra) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Sra
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Srai) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Sra
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Slt) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Slt
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Slti) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Slt
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Sltu) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := readGpReg(rs2)
                    alu.io.op := AluOpcode.Sltu
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Sltiu) {
                    alu.io.a  := readGpReg(rs1)
                    alu.io.b  := imm
                    alu.io.op := AluOpcode.Sltu
                    writeGpReg(rd, alu.io.out)

                    pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Lb) {
                    memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt).asUInt
                    memDataWriteReg := false.B
                    memDataReq      := true.B
                }.elsewhen(op === Opcode.Lbu) {
                    memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt).asUInt
                    memDataWriteReg := false.B
                    memDataReq      := true.B
                }.elsewhen(op === Opcode.Lw) {
                    memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt).asUInt
                    memDataWriteReg := false.B
                    memDataReq      := true.B
                    lswState        := lswSFirstByte
                }.elsewhen(op === Opcode.Sb) {
                    memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt).asUInt
                    memDataInReg    := readGpReg(rd)(7, 0)
                    memDataWriteReg := true.B
                    memDataReq      := true.B
                }.elsewhen(op === Opcode.Sw) {
                    memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt).asUInt
                    memDataInReg    := readGpReg(rd)(7, 0)
                    memDataWriteReg := true.B
                    memDataReq      := true.B
                    lswState        := lswSFirstByte
                }.elsewhen(op === Opcode.Jmp) {
                    // regs
                    writeGpReg(rd, pc.io.out + (WORD_LEN.U / BYTE_LEN.U))

                    // pc
                    pc.io.in    := (pc.io.out.asSInt + imm.asSInt).asUInt
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Jmpr) {
                    val t = pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    writeGpReg(rd, t)
                    val newPC = (readGpReg(rs1).asSInt + imm.asSInt).asUInt & ~1.U(WORD_LEN.W)
                    pc.io.in    := newPC
                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Beq) {
                    // pc
                    when(readGpReg(rs1) === readGpReg(rs2)) {
                        pc.io.in := (pc.io.out.asSInt + imm.asSInt).asUInt
                    }.otherwise {
                        pc.io.in := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    }

                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Bne) {
                    // pc
                    when(readGpReg(rs1) =/= readGpReg(rs2)) {
                        pc.io.in := (pc.io.out.asSInt + imm.asSInt).asUInt
                    }.otherwise {
                        pc.io.in := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    }

                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Blt) {
                    // pc
                    when(readGpReg(rs1).asSInt < readGpReg(rs2).asSInt) {
                        pc.io.in := (pc.io.out.asSInt + imm.asSInt).asUInt
                    }.otherwise {
                        pc.io.in := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    }

                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Bge) {
                    // pc
                    when(readGpReg(rs1).asSInt >= readGpReg(rs2).asSInt) {
                        pc.io.in := (pc.io.out.asSInt + imm.asSInt).asUInt
                    }.otherwise {
                        pc.io.in := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    }

                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Bltu) {
                    // pc
                    when(readGpReg(rs1) < readGpReg(rs2)) {
                        pc.io.in := (pc.io.out.asSInt + imm.asSInt).asUInt
                    }.otherwise {
                        pc.io.in := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    }

                    pc.io.write := true.B
                }.elsewhen(op === Opcode.Bgeu) {
                    // pc
                    when(readGpReg(rs1) >= readGpReg(rs2)) {
                        pc.io.in := (pc.io.out.asSInt + imm.asSInt).asUInt
                    }.otherwise {
                        pc.io.in := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                    }
                    pc.io.write := true.B
                }

                // state
                when(
                    op === Opcode.Lb ||
                        op === Opcode.Lbu ||
                        op === Opcode.Lw ||
                        op === Opcode.Sb ||
                        op === Opcode.Sw
                ) {
                    state := sExec2
                }.otherwise {
                    state := sFetch
                }
            }
            is(sExec2) {
                when(io.memDataDone) {
                    memDataReq      := false.B
                    memDataWriteReg := false.B
                    memDataAddrReg  := 0.U
                    memDataInReg    := 0.U
                    when(op === Opcode.Lb) {
                        writeGpReg(rd, signExtend(io.memDataOut, BYTE_LEN, WORD_LEN))
                        pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                        pc.io.write := true.B
                        state       := sFetch
                    }.elsewhen(op === Opcode.Lbu) {
                        writeGpReg(rd, io.memDataOut)
                        pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                        pc.io.write := true.B
                        state       := sFetch
                    }.elsewhen(op === Opcode.Lw && lswState === lswSFirstByte) {
                        dataBuf         := io.memDataOut
                        memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt + 1.S).asUInt
                        memDataWriteReg := false.B
                        memDataReq      := true.B
                        lswState        := lswSSecondByte
                    }.elsewhen(op === Opcode.Lw && lswState === lswSSecondByte) {
                        val result = Cat(io.memDataOut, dataBuf)
                        writeGpReg(rd, result)
                        pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                        pc.io.write := true.B
                        lswState    := lswSFetch
                        state       := sFetch
                    }.elsewhen(op === Opcode.Sb) {
                        pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                        pc.io.write := true.B
                        state       := sFetch
                    }.elsewhen(op === Opcode.Sw && lswState === lswSFirstByte) {
                        memDataAddrReg  := (readGpReg(rs1).asSInt + imm.asSInt + 1.S).asUInt
                        memDataInReg    := readGpReg(rd)(15, 8)
                        memDataWriteReg := true.B
                        memDataReq      := true.B
                        lswState        := lswSSecondByte
                    }.elsewhen(op === Opcode.Sw && lswState === lswSSecondByte) {
                        pc.io.in    := pc.io.out + (WORD_LEN.U / BYTE_LEN.U)
                        pc.io.write := true.B
                        lswState    := lswSFetch
                        state       := sFetch
                    }
                }
            }
        }
    }
}
